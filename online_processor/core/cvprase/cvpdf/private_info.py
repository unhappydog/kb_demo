...
class indivial(object):
    def basic_info(self, doc, pagenum):
        ...
        subdict['birthday'] = datetime.strptime(
            "".join(re.compile('\d{4}.\d+').findall(subdict['birthday'])).replace('年', '-'), '%Y-%m')
        return subdict
    ...
    def content(self, doc, pagevalue, pagedict):
        ...
        for i in pagedict.keys():
            if i == '求职意向':
                ...
                dict = {'期望工作地区': 'expectedWorkplace', '期望月薪': 'expectedSalary', '目前状况': 'expectedStatus',
                        '期望工作性质': 'expectedWorkNature', '期望从事职业': 'expectedOccupation', '期望从事行业': 'expectedIndustry'}
        ...
        for line in worksplit:
            ...
            if '-' in line[0]:
                ...
                if '至今' in line[0]:
                    ...
                    temp['workEndTime'] = datetime.strptime(
                        "".join(re.compile('\d{4}.\d{2}').findall(line[0].split('-')[1])).replace('.', '-'), '%Y-%m')
            else:
                ...
                if '至今' in line[1]:
                    ...
                    temp['workEndTime'] = datetime.strptime(
                        "".join(re.compile('\d{4}.\d{2}').findall(line[1].split('-')[1])).replace('.', '-'), '%Y-%m')
            try:
            ...
            if newinfo:
                ...
                for line in newinfo:
                    ...
                    if protitle:
                        ...
                        if protime:
                            temp['projectStartTime'] = datetime.strptime(
                                protime.split('-')[0].strip(' ').replace('.', '-'), "%Y-%m")
                            temp['projectEndTime'] = datetime.strptime(
                                protime.split('-')[1].strip(' ').replace('.', '-'), '%Y-%m')
            ...
            for tra in range(0, len(trainlist)):
                ...
                for be in newinfo:
                    if '-' in be:
                        ...
                        if timematch:
                            temp['trainingStartTime'] = datetime.strptime(
                                timematch.split('-')[0].strip(' ').replace('.', '-'), '%Y-%m')
                            temp['trainingEndTime'] = datetime.strptime(
                                timematch.split('-')[1].strip(' ').replace('.', '-'), '%Y-%m')
                        else:
