...
class TerminologyLinker:
    ...
    def _link(self, cv):
        ...
        for property in propertys:
            result[property] = self.link_in_property(cv, property)
            skill_tag += [data for  data in result[property]]

        if cv['skill']:
            if type(cv['skill']) == list:
                result['skill'] = self.link_in_sub_property(cv,'skill',['name'])
                skill_tag += [tag for data in result['skill'] for tag in data.get('name',[]) if 'name' in tag.keys()]
            else:
                result['skill'] = self.link_in_property(cv,'skill')
                skill_tag += [data for data in result['skill']]
        ...
        for k, v in link_dict.items():
            ...
            for sub_property in v:
                if result[k] is None:
                    continue
                skill_tag += [tag for data in result[k] for tag in data.get(sub_property,[]) if 'name' in tag.keys() and tag['name'] != '\n']
        return result, skill_tag

    def skill_tags(self, cv):
        ...
        def if_in_set(x):
            if x['name'] in temp:
                ...
                temp.add(x['name'])
                return True
        skill_tags = [skill_tag['terminology_detail'] for skill_tag in skill_tags if if_in_set(skill_tag)]
        return skill_tags
    ...
    def link_in_sub_property(self, target_object, property_name, sub_propertys):
        ...
        """
        result = []
        sub_object_list = target_object.get(property_name)
    ...
    def linke_with_ac(self, text):
        ...
        result = [
            {'name': data[3][1],
             'start_index': data[0],
             'end_index': data[1]+1,
             'terminology_detail': self.kb_terminology_controller.get_data_by_id(data[3][0]) if self.kb_terminology_controller.get_data_by_id(data[3][0]) else None
