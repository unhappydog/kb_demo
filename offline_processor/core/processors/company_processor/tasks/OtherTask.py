...
class ExtractTask(BaseTask):
    ...
    def fit(self, data):
        ...
        companys = data.apply(lambda x: x[['_id', 'companyName', 'establishedDate', 'companyScale', 'companyLocation', 'brief', 'dom', 'companyType', 'regCapital']])
        companys['name'] = companys['companyName']
        companys.apply(lambda x:self.save_company(x), axis=1)
        [self.save_to_neo4j('member',x) for x in self.members if not self.if_exists('member', x)]
    ...
    def extract_invest_info(self, invest, jd_id):
        ...
        relation = invest
        relation['to_id'] = invest_info['_id']
